ifEnabled("totalcomments", function() {
	...
	function buildAuthorList() {
		...
		pos.top += 18;
		$("#author_selector").empty().append("<ul />").css({
			top: pos.top + "px",
			left: pos.left + "px"
		}).show();
	...
	$("<tr />", {
		id: "totalcomments"
	}).insertAfter("#content_left_inner table.category tr");
	$("#totalcomments").append($("<td />", {
		class: "system",
		colspan: 4
	}).
	append($("<span />", {
		id: "totalcommentsspan",
		text: "TotalComments: "
	})));
	...
	$("#totalcommentsspan").append($("<span />", {
		id: "min_rating",
		text: "рейтинг:",
		title: "Показывать комментарии с рейтингом не ниже указанного",
		className: "filter"
	})).append($("<input />", {
		css: {
			width: "30px"
		...
		var rating = Number(this.value);

		$("#js-commentsHolder .post" + current_selector).show();
		$("#min_rating").removeClass("selected");
		if (!isNaN(rating)) {
			$("#js-commentsHolder div.post").each(function() {
				var commentRating = Number($("#{0}  div.vote span em".fmt(this.id)).text());

				if (commentRating < rating) {
					$(this).hide();
				}
			});
			$("#min_rating").addClass("selected");
		}
	}))
});
