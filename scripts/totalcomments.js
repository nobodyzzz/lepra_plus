...
var config = {
	max_inbox_rating: 0,
	// Открывать комментарии "инбокс ми",
...
for (i = 0; i < modes.length; i++) {
	...
	link.style.borderBottom = "1px dotted #000000";
	link.setAttribute("mode", mode.name);
	link.appendChild(document.createTextNode(mode.title));
	link.addEventListener("click", function(event) {
		current_mode = event.target.getAttribute("mode");
		setViewMode(event.target.getAttribute("mode"));
		return false;
	},
	false);
...
addGlobalStyle('.ltc_button:hover {background: #F6EFD2; border: 0px;}');
...
function isNotFuckingInboxRequest(comment) {
	return ! isFuckingInboxRequest(comment)
}
